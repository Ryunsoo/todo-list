...
let renderUser = (e) => {
	e.preventDefault(); //브라우저의 구현에 의해 처리되는 기존의 액션이 동작하지 않게되고, 그 결과 이벤트가 발생하지 않게됩니다.
...
let convertMainDiv = (username) => {
	...
	inpName.className = 'inp_todo'; //todo list 입력 시 다른 event를 걸어주기위해 className 변경
...
let removeSchedule = (event) => {
	...
	/*let lastPage = Math.ceil(res.length/8); //추가
	if(lastPage < curPage){
		curPage -= 1;
	}*/
	
	renderPage(curPage);
	document.querySelector('.current-page').textContent = curPage;
}
